...
class Manage_MASC:
                ...
                try:
                    with open(json_file_path, 'r') as file:
                        ...
                        if bounds:
                            ...
                            if all_children:
                                keywords = ""
                                with open(os.path.join(category_full_path, all_files_full[i]), 'r') as full_file:
                                ...
                                for child in all_children:
                                    ...
                                    if Manage_MASC.list_clickable.__contains__(child["componentLabel"]) and child["clickable"].lower() == "true":
                                ...
                                list_keywords = ManageKeywords.keywords_dictionary[category_semantic]
    ...
    def run_masc(self):
        path_json_test_full = "your_path_json_test_full"
        path_json_output = "path_json_output"

        # Get features for both selected and full feature sets
        list_vector_selected, list_features_selected, list_labels_selected = Manage_MASC.get_all_features(
            path_json_test, path_json_test_full, is_selected_features=True
        )

        list_vector_full, list_features_full, list_labels_full = Manage_MASC.get_all_features(
            path_json_test, path_json_test_full, is_selected_features=False
        )

        # Write results to CSV files
        with open(os.path.join(path_json_test, "Allvectors.csv"), 'w', newline='') as f:
